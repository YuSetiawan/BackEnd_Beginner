...
const insertProduct = (data) => {
  const {id, name, stock, price, image, rate, brand, id_category} = data;
  return Pool.query(`INSERT INTO product(id, name, stock, price, image, rate, brand, id_category)
  VALUES (${id}, '${name}', ${stock}, ${price}, '${image})', ${rate}, '${brand}', ${id_category})`);
};

const updateProduct = (data) => {
  const {id, name, stock, price, image, rate, brand, id_category} = data;
  return Pool.query(`UPDATE product SET name='${name}', stock=${stock}, price=${price}, image='${image}', rate=${rate}, brand='${brand}', id_category=${id_category} WHERE id =${id}`);
};
